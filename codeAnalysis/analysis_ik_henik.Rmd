...
The plot shows the raw cursor trajectories from the participant with `p_num==53`, and we can see that the trajectories were tracked in a somewhat twisted way, that is, the coordinate system in OpenSesame was centered in middle of the screen and going upwards toward the response boxes decreased the y-coordinate. 
...
plot_data = mt$data %>% 
  left_join(
    mt$measures %>% select(mt_id, MAD, AUC, sample_entropy, xpos_flips),
    by = "mt_id"
  ) 
...
plot_grid(
  ...
  plot_data %>%
    ...
    stat_summary(aes(group=1),fun = "mean", geom = "line", alpha = 1, color="steelblue"),
  # plot_data %>% 
  #   ggplot(aes(x=t_condition,y=xpos_flips,group=p_num))+
  #   geom_point(alpha = .1)+
  #   stat_summary(fun = "mean", geom = "line", alpha = .1)+
  #   stat_summary(aes(group=1),fun = "mean", geom = "line", alpha = 1, color="steelblue"),
  plot_data %>% 
    ggplot(aes(x=t_condition,y=sample_entropy,group=p_num))+
    geom_point(alpha = .1)+
    stat_summary(fun = "mean", geom = "line", alpha = .1)+
    stat_summary(aes(group=1),fun = "mean", geom = "line", alpha = 1, color="steelblue"),
  nrow = 1,
...
plot_grid(
  mt %>%
    mt_subset(c_correct==1 & t_condition=="congruent") %>% 
    mt_plot(
    use2 = "prototyping",
    facet_col = "prototype_label",
    alpha =.05),
  mt %>%
    mt_subset(c_correct==1 & t_condition=="incongruent") %>% 
    mt_plot(
    use2 = "prototyping",
    facet_col = "prototype_label",
    alpha =.05),
...
For instance, other factors might be the difference between the two numbers, or the congruency of the previous trial (is `t_congruency` the same or does it switch from trial to trial).

## Investigate movement quality
Check, for instance, which participants show less movement quality and this influences the results by removing these participants from the analyses.
